...
from graph_generator.graphs.conversions import transform_four_axes, transform_three_axes, transform_two_axes
...
def make_two_dimensional_chart(data: ToGraph, config: Graph):
    ...
    return sns.lineplot(data=df, x=config.x_axis, y="y", hue=config.color)
...
def make_three_dimensional_chart(data: ToGraph, config: Graph):
    ...
    return sns.lineplot(data=df, x=config.x_axis, y="y", hue=config.color, style=config.style)
